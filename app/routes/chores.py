...
def get_target_user_id(db):
    if current_user.is_parent:
        child_id = request.args.get("child_id", type=int) or request.form.get(
            "child_id", type=int
        )
...
def index():
    ...
    children = None
    parent_record_map = None
    if current_user.is_parent:
        children = get_family_children(db)
        parent_record_map = build_record_map(db, current_user.id, month_str)
    ...
    return render_template(
        ...
        children=children,
        parent_record_map=parent_record_map,
        prev_year=prev_year,
...
def toggle():
    ...
    # report_id: 画面に表示中の子供（集計・金額の再計算対象）。親が自分の分を
    # トグルするときは target_id（親自身）と異なるため別で受け取る。
    report_id = data.get("report_user_id", target_id)
    ...
    if current_user.is_parent:
        ...
        if not is_self and not verify_child_ownership(db, int(target_id)):
            return jsonify({"error": "権限がありません"}), 403
        if not verify_child_ownership(db, int(report_id)):
            return jsonify({"error": "権限がありません"}), 403
    ...
    """,
        (target_id, chore_type_id, record_date_str),
    ).fetchone()
    ...
    """,
        (chore_type_id, record_date_str),
    ).fetchone()["cnt"]

    # サマリー再計算（月全体、画面に表示中の子供分）
    ...
    for ct in chore_types:
        row = db.execute(
            ...
            WHERE user_id=? AND chore_type_id=?
              AND strftime('%Y-%m', record_date)=?
        """,
            (report_id, ct["id"], month_str),
        ).fetchone()
        summary[str(ct["id"])] = int(row["cnt"])

    # 獲得金額も再計算
    year_int, month_int = int(month_str[:4]), int(month_str[5:7])
    new_chore_pay = calc_chore_pay(int(report_id), year_int, month_int)
