...
def run():
    ...
    jarvis_key = _os.environ.get("JARVIS_KEY", "jarvis-2026")
    req_key = request.args.get("key", "")
    if req_key != jarvis_key:
    ...
    db.execute(
        "INSERT INTO users (name, username, password_hash, role) VALUES (?,?,?,'parent')",
        ("お父さん", "parent", generate_password_hash("demo1234")),
    )
    db.commit()
    parent_id = db.execute("SELECT id FROM users WHERE username='parent'").fetchone()[
        "id"
    ]
    ...
    # 子ユーザー（family_id付き）
    db.execute(
        "INSERT INTO users (name, username, password_hash, role, grade, family_id) VALUES (?,?,?,'child',4,?)",
        ("たろう", "taro", generate_password_hash("taro1234"), family_id),
    )
    db.execute(
        "INSERT INTO users (name, username, password_hash, role, grade, family_id) VALUES (?,?,?,'child',2,?)",
    ...
    hanako_id = db.execute("SELECT id FROM users WHERE username='hanako'").fetchone()[
        "id"
    ]
        ...
        for cid in chore_ids:
            for uid in [taro_id, hanako_id]:
                if random.random() < 0.5:
                    try:
                        db.execute(
                            "INSERT INTO chore_records (user_id,chore_type_id,record_date,checked_by) VALUES (?,?,?,?)",
                            (uid, cid, d, parent_id),
                        )
                    except Exception:
                        pass
    ...
    for i, name in enumerate(["国語", "算数", "理科", "社会", "体育", "音楽", "図工"]):
        db.execute(
            "INSERT OR IGNORE INTO subjects (name,sort_order) VALUES (?,?)", (name, i)
        )
    db.commit()
    ...
    for sid in [r["id"] for r in db.execute("SELECT id FROM subjects").fetchall()]:
        try:
            db.execute(
                "INSERT OR IGNORE INTO grade_records (user_id,subject_id,year,term,eval_1,entered_by) VALUES (?,?,?,1,?,?)",
    ...
    for days_ago, uid in [(15, taro_id), (10, taro_id), (5, hanako_id)]:
        d = (today - timedelta(days=days_ago)).isoformat()
        db.execute(
            "INSERT INTO finance_records (user_id,record_date,type,category,amount,created_by) VALUES (?,?,'income','給料',1000,?)",
            (uid, d, parent_id),
        )
        db.execute(
            "INSERT INTO finance_records (user_id,record_date,type,category,amount,created_by) VALUES (?,?,'expense','お菓子',150,?)",
            (uid, d, parent_id),
        )
...
def patch_families():
    """family_idのないユーザーにfamilyを作成するパッチ（JARVISキー認証必須）"""
    import os as _os
    ...
    for parent in parents:
        ...
        db.execute(
            "UPDATE users SET family_id=? WHERE role='child' AND (family_id IS NULL)",
            (family_id,),
        )
        db.commit()
        patched += 1
