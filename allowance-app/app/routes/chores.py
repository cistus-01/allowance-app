...
def build_record_map(db, target_id, month_str):
    ...
    rows = db.execute('''
        SELECT
            record_date,
            chore_type_id,
            COUNT(*) AS checker_count,
            SUM(CASE WHEN user_id = ? THEN 1 ELSE 0 END) AS my_check
        FROM chore_records
        WHERE strftime('%Y-%m', record_date) = ?
        GROUP BY record_date, chore_type_id
...
def toggle():
    ...
    if current_user.is_parent:
        if record_date.year != today.year or record_date.month != today.month:
            return jsonify({'error': '親も当月分のみ編集できます'}), 403
